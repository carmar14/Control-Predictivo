...
Ad=[1.7 -0.5;0.4 0.8]; % discrete time system xk+1=Axk+Buk
...
% initial condition
x0=[-1;1];%[-1,-1];%[0.07501;-0.3327];%[0.3;0.1];
...
plot(Xset,'Color',[0.8 0.8 0.8])
hold on
plot(Xred,'Color',[0.9 0.9 0.9])
plot(set_X{end},'Color',[0.5 0.5 0.5])
...
H=[Bbig'*Qbig*Bbig+Rbig Bbig'*Qbig*Abig;Abig'*Qbig*Bbig Abig'*Qbig*Abig];
...
%     b=ubounds*ones(horizon*2,1);
    ...
    % xopt-x0\in Z
    % mRPIsetH*(xopt-x0)<=mRPIsetK
    % mRPIsetH*xopt<=mRPIsetK+mRPIsetH*x0
    
    A=blkdiag(kron(eye(horizon),UredH),mRPIsetH);
    b=[kron(ones(horizon,1),UredK);mRPIsetK+mRPIsetH*x0];
...
% XredHbig*(Abigx0opt+Bbig*U)<=XredKbig


  XredHbig=kron(eye(horizon+1),XredH);
  XredKbig=kron(ones(horizon+1,1),XredK);
  
  mask=[zeros(size(Ad,2),size(Ad,2)*horizon) eye(size(Ad))];
  lastlines=mask*[Bbig Abig];
  %lastlines*[U xopt]=lastx
  % HH*lastx<=KK
  % HH*lastlines*[U xopt]<=KK
  A=[A;[XredHbig*Bbig XredHbig*Abig];HH*lastlines];
  b=[b;XredKbig;KK];
    ...
    f=zeros(1,size(Bbig,2)+size(Ad,2));
    f=f';
    
    [U,FVAL,EXITFLAG]=quadprog(2*H,f,A,b);
    traj=[Bbig Abig]*U;
    traj_r=reshape(traj,2,horizon+1);
    x0opt=U(end-size(Ad,2)+1:end);
    se_t=plot(mRPIset+x0,'Color','g');
    set(se_t,'FaceAlpha',0.2);
    set(se_t,'EdgeAlpha',0.5);
    plot(x0opt(1),x0opt(2),'rp')
    plot(x0(1),x0(2),'b*')
    tpl=plot(traj_r(1,:),traj_r(2,:),'k*');
...
%         X_re=[reshape(X,2,horizon+1)'];
        ...
        xtemp=Ad*x0+Bd*(U(1,:)-K*(x0-x0opt))+0.1*(rand*2-1);
%         xx=[xx;X_re(2,:)];
        xx=[xx;xtemp'];
        uu=[uu;U(1,:)];
    end
...
figure(1)
fin=plot(mRPIset,'Color','y')
set(fin,'FaceAlpha',0.7);
plot(xx(:,1),xx(:,2),'b*')
axis tight